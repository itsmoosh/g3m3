...
c
c      evolves the electron pressure equation
c
c
...
c
c      standard runge-kutta push for ion equations
c
...
c
c       maximum ion cyclotron frequency in normalized units
c           that that can be rsolved
        bmax=0.20*rmassq/delt/reynolds
c
c        set distance scales
c
...
c           that that can be rsolved
        ...
        bmax=0.20*rmassq/delt/reynolds
c
c parallelizes loop RW, Aug. 17, 2004
c$omp  parallel do 
      do k=1,nz
...
c
      ...
      enddo          ! i loop
c
c     begin main loop for finding all terms at n+1/2 from i=2,nx-1
c           and from j=2,ny-1 using boundary conditions
 
c parallelizes the loop BU, Oct. 10 2002
c$omp  parallel do 
       do k=2,nz-1
       km=k-1
       kk=k+1
...
c        find estimates for the fluid at n+1/2
...
c
     ...
     +                    /dzt ) )
c
c       add increments from spatial derivatives
c
        abx=bsx(i,j,k)
        aby=bsy(i,j,k)
        abz=bsz(i,j,k) 
...
c
c       using straight electric field approach
c
...
c
c
c       add in JxB and grad P forces
c       qjcrossb_x=(cury(i,j,k)*abz-curz(i,j,k)*aby)
c       qjcrossb_y=-(curx(i,j,k)*abz-curz(i,j,k)*abx)
c       qjcrossb_z=(curx(i,j,k)*aby-cury(i,j,k)*abx)
c
c       qpx(i,j,k,m)=aqpx+delt*(qjcrossb_x-qgradp_x) 
c       qpy(i,j,k,m)=aqpy+delt*(qjcrossb_y-qgradp_y) 
c       qpz(i,j,k,m)=aqpz+delt*(qjcrossb_z-qgradp_z)
c
c       add gravity
c
...
c
c       pressure equations: isotropic components
c
c
...
c
c       add in anisotropic components
c  
...
c
     ...
     +            -wrkqpresyz(i,j,k,m)*abx )
c
c       pressure equations: offdiagonal elements
c
...
c
     ...
     +              rx,ry,rz) 
c
c      standard runge-kutta time step
c
...
c
     ...
     +              rx,ry,rz)
c
c     this calculates the current associated with the perturbed B field
c          i.e. curz= dby/dx - dbx/dy
c
      dimension bx(nx,ny,nz,ngrd),by(nx,ny,nz,ngrd),bz(nx,ny,nz,ngrd),
...
c
      ...
      dyt=2.*ry
      dzt=2.*rz
c parallelizes loop RW, Oct. 23, 2002
c$omp  parallel do 
      do k=2,nz-1
      km=k-1
      kp=k+1
...
c
     ...
     +         - (bx(i,jp,k,m)-bx(i,jm,k,m))/dyt
       ...
       enddo
c
c     following boundary conditions are set so that no forward
c      communication , i.e. same x required
c      and symmetry between k=1 and k=2
c
c     set boundary regions - bottom and top panels
c
      nx1=nx-1
      ny1=ny-1
      nz1=nz-1
...
c
      ...
      enddo
c
c       set boundary regions - front and back
c
c parallelizes loop RW, Oct. 23, 2002
c$omp  parallel do 
      do k=2,nz1
...
c
      ...
      enddo
c
c       set boundary regions - left and right
c
c parallelizes loop RW, Oct. 23, 2002
c$omp  parallel do 
      do j=2,nz1
...
c
c      calculates the space-charge fields in the plasma
c
...
c
      d_min=0.001
c
c     take the divergence of E and correct
c          for curl B_0 errors
c     
      do k=2,nz-1
      km=k-1
      kp=k+1
...
c
      ...
      enddo
c
c      set flank bounday conditions
c
      nx1=nx-1
      ny1=ny-1
      nz1=nz-1
...
c
     ...
     +       rx,ry,rz)
c
c      calculates the surface magnetic field bs
c      and the body electric field eb
c
      dimension bsx(nx,ny,nz),bsy(nx,ny,nz),bsz(nx,ny,nz),
     +      efldx(nx,ny,nz),efldy(nx,ny,nz),efldz(nx,ny,nz),
     +      curx(nx,ny,nz),cury(nx,ny,nz),curz(nx,ny,nz),
...
c
      ...
      enddo
c
c      add in ionospheric resistance
c
...
c
c      boundary condtions for terrestrial surface currents
c
c     if(m.le.mbndry)then
c      do n=1,nummid(m)
c        i=ijmid(m,1,n)
c        j=ijmid(m,2,n)
c        k=ijmid(m,3,n)
...
c
c     set an array v of total dimension n to value a
c
      dimension v(1)
c
c parallelizes loop RW, Oct. 23, 2002
...
c
     ...
     +     nx,ny,nz,ngrd,m,chirho,chipxyz,chierg,delt)
c
c     apply the Ladipus smoothing technique for particular ion component
c
...
c
     ...
     +           sin_tilt,cos_tilt
c
c     equal waiting irrespective of grid size
c
      deltz=delt
      delty=delt
      deltx=delt
...
c
c     apply flux correction smoothing technique
c
      dimension qrho(nx,ny,nz,ngrd),qpresx(nx,ny,nz,ngrd),
     +     qpresy(nx,ny,nz,ngrd),qpresz(nx,ny,nz,ngrd),
     +     qpresxy(nx,ny,nz,ngrd),qpresxz(nx,ny,nz,ngrd),
...
c
c
c      write(6,*)'in flux correct with',nx,ny,nz,ngrd,m,chirho,
c    +              diferg,xspac
c
       rx=xspac(m)
       ry=xspac(m)
...
c
     ...
     +          delta,delta1,fn)
c
c     applies flux correction smoothing to field quantities
c           oldpx is the qunatity at the start of the time step
c           wrkpx is the unsmoothed qunatity
c           px is the final product
c     wrk arrays assumed to have dimension larger than nx,ny,nz
c
      dimension px(nx,ny,nz,ngrd),oldpx(nx,ny,nz,ngrd),
     +            wrkpx(nx,ny,nz,ngrd),
     +        delta(nx,ny,nz),delta1(nx,ny,nz),fn(nx,ny,nz)
c
c         step 1:   diffuse initial in x direction
c
c$omp  parallel do
      do k=1,nz
      do j=1,ny
...
c$omp  parallel do
      ...
      enddo
c
c     calculate the difference  between x nearest grid points
c
cc$omp  parallel do
      do k=1,nz
      do j=1,ny
...
c        px(i,j,k,m)=px(i,j,k,m)-fn(i,j,k)+fn(i1,j,k)  ! x-smooth only
        enddo
      enddo
      enddo
c
c         step 2:   diffuse initial in y -dreiction
c
c$omp  parallel do
      do k=1,nz
      do j=2,ny-1
...
c
        do j=2,ny-1
        j1=j-1
c        wrkpx(i,j,k,m)=px(i,j,k,m)-fn(i,j,k)+fn(i,j1,k) ! for x-y-z smoothing
         px(i,j,k,m)=px(i,j,k,m)-fn(i,j,k)+fn(i,j1,k) ! for x-y smoothing
        enddo
      enddo
      enddo
c
c         step 3:   diffuse initial in z -dreiction
c
c
cc$omp  parallel do
c     do k=2,nz-1
...
c     do j=1,ny
c      do i=1,nx
c        px(i,j,k,m)=wrkpx(i,j,k,m)+chipx*(
c    +     oldpx(i,j,kp,m)+oldpx(i,j,km,m)
c    +      -2.*oldpx(i,j,k,m))
c      enddo
c     enddo
c     enddo
...
c     do j=1,ny
c     do i=1,nx
c      px(i,j,1,m)=wrkpx(i,j,1,m)
c      px(1,j,nz,m)=wrkpx(1,j,nz,m)
c     enddo
c     enddo
c     calculatethe difference  between x nearest grid points
c
c     do j=1,ny
c     do i=1,nx
c       do k=1,nz-1
c         delta(k)=wrkpx(i,j,k+1,m)-wrkpx(i,j,k,m)   !non-diffuse n+1 solution
c         delta1(k)=px(i,j,k+1,m)-px(i,j,k,m)   !diffused n+1 solution
c        enddo
c        enddo
c
c       delta(nz)=delta(nz-1)
c       delta1(nz)=delta1(nz-1)
c
c       do k=2,nz-1
c        s=sign(1.,delta(k))
c        fn(k)=s*amax1(0.,amin1(s*delta1(k-1),chipx*abs(delta(k)),
c    +                            s*delta1(k+1)))
c       enddo
c
c        fn(1)=fn(2)
...
c
c       do k=2,nz-1
c        px(i,j,k,m)=px(i,j,k,m)-fn(k)+fn(k-1)
c       enddo
c     enddo
c     enddo
c
...
c
c      save initial unperturbed values in temp array
c
cc$omp  parallel do 
      do k=1,nz
      do j=1,ny
      ...
      enddo
c
c         step 1:   diffuse initial in all directions
c$omp  parallel do 
      do k=2,nz-1
      kp=k+1
      km=k-1
...
c        px(i,j,k,m)=px(i,j,k,m)-fnx(i)+fnx(i1)  ! x-smooth only
        enddo
      enddo
      enddo
c
c
c
c     calculate the difference  between y nearest grid points
c
cc$omp  parallel do 
      do k=1,nz
      do i=1,nx
...
c        px(i,j,k,m)=tempx(i,j,k)-fny(j)+fny(j1) ! for x-y smoothing
        enddo
      enddo
      enddo
c
c     calculatethe difference  between z nearest grid points
c
cc$omp  parallel do
      do j=1,ny
      do i=1,nx
...
c
c     apply straight artifical diffusion : 
c
...
c
     ...
     +    +wrk hpres(i,j,kp,m)+wrkhpres(i,j,km,m)
     +        -6.*wrkhpres(i,j,k,m) )
c
c       species 3
c
...
c
     ...
     +        -6.*wrkbz(i,j,k,m) )
       ...
       enddo
c
c     reset time index so that desired quanties lie at nt1 rather than nt2
c
c     ntnew=nt2
c     nt2=nt1
c     nt1=ntnew
...
c
     ...
     +           nx,ny,nz)
c
c     force smoothing of surface currents to see if we can
c             get better graphics
c
...
c
c     applies straight diffusion
c
      dimension px(nx,ny,nz,ngrd),wrkpx(nx,ny,nz,ngrd)
c
c     write(6,*)'ssmooth',nx,ny,nz,ngrd,m,chipx
c
c         step 1:   diffuse 3-D direction
c
c$omp  parallel do 
      do k=2,nz-1
      do j=2,ny-1
...
c
     ...
     +    grd_xmin,grd_xmax,grd_ymin,grd_ymax,grd_zmin,grd_zmax)
c
c     this subroutine corrects for possible non-divergence of B
c         it integrates from the wind boundary
c
...
c
     ...
     +           grd_zmin(ngrd),grd_zmax(ngrd)
c
c     set limit of x integration depending on when you
c         hit magnetopause
c
      r_max=4.5*srho
      r_min=0.15*srho
...
c
c     this routine applies boundary conditions around the edges
c     of the system and at any irregular boundaries
c
      dimension 
...
c
c
c     set surface conditions around earth
c      withinterior temperature as constant
c
c     write(6,*)'inside bndry inner with',mbndry
...
c
      ...
      r_rot2=r_rot**2
c
c     set ionospheric boundary points - plasma quantities defined
c
      do n=1,numsrf(m)
c
...
c
         ...
         k=ijsrf(m,3,n)
c
c         set rotational speeds
c
         ay=grd_ymin(m)+dy*(j-1)
         ax=grd_xmin(m)+dx*(i-1)
         rx=ax*re_equiv
         ry=ay*re_equiv
c        vfrac=(r_rot2)/(r_rot2+ rx**2+ry**2)
         vfrac=1.
         rvy=rx*v_rot*vfrac
         rvx=-ry*v_rot*vfrac
         rvz=0.
...
c
c        iostropic conditions: set off axis pressures to zero
c
         qpresxy(i,j,k,m)=0.
         qpresxz(i,j,k,m)=0.
         qpresyz(i,j,k,m)=0.
...
c
      enddo
c
c     set atmospheric boundary points - specify magnetic field profile
c
      do n=1,nummid(m)
c
c        get coords of point
...
c
c     Do x boundary conditions : set oxygen in solar wind
c       at 2.5%
c
...
c     do 30 i=1,2
c
c       wind boundary conditions at i=1
c
        qrho(i,j,k,m)=srho
        qpx(i,j,k,m)=spx
        qpy(i,j,k,m)=spy
...
c
c
c       set bx of wind to zero, by to the wind values
c          AND subtract any geomagnetic field
c           
        bx(i,j,k,m)=sbx_wind-bx0(i,j,k,m) 
        by(i,j,k,m)=sby_wind-by0(i,j,k,m)
        bz(i,j,k,m)=sbz_wind-bz0(i,j,k,m)
c
c       bx(i,j,k,m)=bx(2,j,k,m)
c       by(i,j,k,m)=by(2,j,k,m)
c       bz(i,j,k,m)=bz(2,j,k,m)
c
   30 continue
c
c

c     Do x boundary conditions - back wall
c
      nx1=nx-1
      do 40 j=1,ny
      do 40 k=1,nz
...
c
c       continuous boundary condition
c
...
c    +             +(by0(i1,2,k,m)-by0(i,1,k,m))
        bx(i,1,k,m)=bx(i1,2,k,m)
c    +              +(bx0(i1,2,k,m)-bx0(i,1,k,m))
        bz(i,1,k,m)=bz(i1,2,k,m)
c    +              +(bz0(i1,2,k,m)-bz0(i,1,k,m))
c
...
c    +              +(by0(i1,ny1,k,m)-by0(i,ny,k,m))
        bx(i,ny,k,m)=bx(i1,ny1,k,m)
c    +               +(bx0(i1,ny1,k,m)-bx0(i,ny,k,m))
        bz(i,ny,k,m)=bz(i1,ny1,k,m)
c    +              +(bz0(i1,ny1,k,m)-bz0(i,ny,k,m))
   60 continue
c
c     Do z boundary conditions 
c
      nz1=nz-1
      do 70 i=2,nx
c     i1=i-1
...
c    +            +bz0(i1,j,nz1,m)-bz0(i,j,nz,m)
        bx(i,j,nz,m)=bx(i1,j,nz1,m)
c    +            +bx0(i1,j,nz1,m)-bx0(i,j,nz,m)
        by(i,j,nz,m)=by(i1,j,nz1,m)
c    +            +by0(i1,j,nz1,m)-by0(i,j,nz,m)
   70 continue
c
c       symmetry boundary conditions at k=1 and k=2
c
      do 80 i=2,nx
      do 80 j=1,ny
c
...
c       set bx of wind to zero, by to the wind values
c          AND subtract any geomagnetic field
c       force bz to be positive to attain equilibrium
c          
   ...
   80 continue
c
c     do corner lines at left-right back wall
c
      do 110 k=2,nz-1
...
c
  110 continue         
c
c     do corner lines at up and bottom back wall
c
      do 115 j=2,ny-1
...
c        k=ijsrf(3,n)
c
c        qrho(i,j,k,m)=parm_srf(m,1,n)
c        hrho(i,j,k,m)=parm_srf(m,2,n)
c        orho(i,j,k,m)=parm_srf(m,3,n)
c        qpres(i,j,k,m)=parm_srf(m,4,n)
c        hpres(i,j,k,m)=parm_srf(m,5,n)*0.9
c        opres(i,j,k,m)=parm_srf(m,6,n)
c        epres(i,j,k,m)=parm_srf(m,7,n)
c
c        qrho(i,j,k,m)=0.5*qrho(i,j,k,m)+0.5*parm_srf(m,1,n)
c        hrho(i,j,k,m)=0.5*hrho(i,j,k,m)+0.5*parm_srf(m,2,n)
c        orho(i,j,k,m)=0.5*orho(i,j,k,m)+0.5*parm_srf(m,3,n)
c
c        qpres(i,j,k,m)=0.5*qpres(i,j,k,m)+0.5*parm_srf(m,4,n)
c        hpres(i,j,k,m)=0.5*hpres(i,j,k,m)+0.5*parm_srf(m,5,n)
c        opres(i,j,k,m)=0.5*opres(i,j,k,m)+0.5*parm_srf(m,6,n)
c        epres(i,j,k,m)=0.5*epres(i,j,k,m)+0.5*parm_srf(m,7,n)
c
c        hpx(i,j,k,m)=0.5*hpx(i,j,k,m)+0.5*rvx*parm_srf(m,2,n)
c        hpy(i,j,k,m)=0.5*hpy(i,j,k,m)+0.5*rvy*parm_srf(m,2,n)
c        hpz(i,j,k,m)=0.5*hpz(i,j,k,m)+0.5*rvz*parm_srf(m,2,n)
c        opx(i,j,k,m)=0.5*opx(i,j,k,m)+0.5*rvx*parm_srf(m,3,n)
c        opy(i,j,k,m)=0.5*opy(i,j,k,m)+0.5*rvy*parm_srf(m,3,n)
c        opz(i,j,k,m)=0.5*opz(i,j,k,m)+0.5*rvz*parm_srf(m,3,n)
c        qpx(i,j,k,m)=0.5*qpx(i,j,k,m)+0.5*rvx*parm_srf(m,1,n)
c        qpy(i,j,k,m)=0.5*qpy(i,j,k,m)+0.5*rvy*parm_srf(m,1,n)
c        qpz(i,j,k,m)=0.5*qpz(i,j,k,m)+0.5*rvz*parm_srf(m,1,n)
c
c
c        opx(i,j,k,m)=orho(i,j,k,m)*rvx
c        opy(i,j,k,m)=orho(i,j,k,m)*rvy
c        opz(i,j,k,m)=orho(i,j,k,m)*rvz
c        hpx(i,j,k,m)=hrho(i,j,k,m)*rvx
c        hpy(i,j,k,m)=hrho(i,j,k,m)*rvy
c        hpz(i,j,k,m)=hrho(i,j,k,m)*rvz
c        qpx(i,j,k,m)=qrho(i,j,k,m)*rvx
c        qpy(i,j,k,m)=qrho(i,j,k,m)*rvy
c        qpz(i,j,k,m)=qrho(i,j,k,m)*rvz
c
c      enddo
c
c     set atmospheric boundary points - specify magnetic field profile
...
c
         epres(i,j,k,m)=parm_mid(m,7,n)
c
c        qrho(i,j,k,m)=amax1(0.25*qrho(i,j,k,m),parm_mid(m,1,n))
c        hrho(i,j,k,m)=amax1(0.25*hrho(i,j,k,m),parm_mid(m,2,n))
c        orho(i,j,k,m)=amax1(0.25*orho(i,j,k,m),parm_mid(m,3,n))
c
c        qpres(i,j,k,m)=amax1(0.25*qpres(i,j,k,m),parm_mid(m,4,n))
c        hpres(i,j,k,m)=amax1(0.25*hpres(i,j,k,m),parm_mid(m,5,n))
c        opres(i,j,k,m)=amax1(0.25*opres(i,j,k,m),parm_mid(m,6,n))
c        epres(i,j,k,m)=amax1(0.25*epres(i,j,k,m),parm_mid(m,7,n))
c
c
         opx(i,j,k,m)=orho(i,j,k,m)*rvx
         opy(i,j,k,m)=orho(i,j,k,m)*rvy
...
c
c
c        qrho(i,j,k,m)=amax1(0.10*qrho(i,j,k,m),parm_zero(m,1,n))
c        hrho(i,j,k,m)=amax1(0.10*hrho(i,j,k,m),parm_zero(m,2,n))
c        orho(i,j,k,m)=amax1(0.10*orho(i,j,k,m),parm_zero(m,3,n))
c
c        qpres(i,j,k,m)=amax1(0.05*qpres(i,j,k,m),parm_zero(m,4,n))
c        hpres(i,j,k,m)=amax1(0.05*hpres(i,j,k,m),parm_zero(m,5,n))
c        opres(i,j,k,m)=amax1(0.05*opres(i,j,k,m),parm_zero(m,6,n))
c        epres(i,j,k,m)=amax1(0.05*epres(i,j,k,m),parm_zero(m,7,n))
c
        opx(i,j,k,m)=orho(i,j,k,m)*rvx
        opy(i,j,k,m)=orho(i,j,k,m)*rvy
        opz(i,j,k,m)=orho(i,j,k,m)*rvz
...
c
     ...
     +       grd_zmin,grd_zmax)
c
c      sputtered or ram produced ionosphere
c
      dimension grd_xmin(ngrd),grd_xmax(ngrd),
     +          grd_ymin(ngrd),grd_ymax(ngrd),
     +          grd_zmin(ngrd),grd_zmax(ngrd)
c
     ...
     +        parm_zero(mbndry,7,mzero)  
c
c      write(6,*)'bndry_moon values',
c    +       qden_moon,hden_moon,oden_moon,tempi,gamma,
c    +       ti_te_moon,xmoon,ymoon,zmoon,rmoon,offset,alpha_m
c      write(6,*)mbndry,msrf,mmid,mzero
c
c      set scale lengths
c
       dx=(grd_xmax(m)-grd_xmin(m))/(nx-1.)
...
c
c       set conditions at moon's new position
c
          do k=1,nz
...
c
          ...
          enddo
c
c         write(6,*)'Total pts',ntot
c         write(6,*)'Moon bndry_m pts',m,numsrf(m),nummid(m),numzero(m)
c
      return 
      end
...
c
c     this routine applies boundary conditins to all grid types
c     of the system and at any irregular boundaries
c
c
      dimension 
...
c
     ...
     +           grd_zmin_n(ngrd_n),grd_zmax_n(ngrd_n)
c
c    fills in the core of the main grid in position mb
c       using data from the finer (small) grid in position ms
c
      mb=main_ngrd
      ms=ngrd_n
c
c     set limits for coarse grid
c
       ...
       do ks=1+nskip,nz_n-nskip,nskip
        ...
        do js=1+nskip,ny_n-nskip,nskip
         ...
         do is=1+nskip,nx_n-nskip,nskip
          ib=nbx1+(is-1)/nskip
c 
c         rho(ib,jb,kb,mb)=rho_n(is,js,ks,ms)
c
...
c
c     this routine applies boundary conditions to all grid types
c     of the system and at any irregular boundaries
c
c
      dimension 
...
c
     ...
     +           grd_ymin,grd_ymax,grd_zmin,grd_zmax)
c
c      m is the coring subject grid index
c
     ...
     +          grd_zmin(ngrd),grd_zmax(ngrd)
c
c    fills in the core of the coarser (big) grid in position nb
c       using data from the finer (small) grid in position ns
c
c     set limits for coarse grid
c
      mb=m+1
...
c$omp  parallel do
       do  ks=1+nskip,nz-nskip,nskip
        ...
        do  js=1+nskip,ny-nskip,nskip
         ...
         ib=nbx1+(is-1)/nskip
c
c       old version with no averaging
c        rho(ib,jb,kb,mb)=rho(is,js,ks,ms)
c
c       smooth to make compatible with coarse grid
c
...
c
     ...
     +           grd_zmin(ngrd),grd_zmax(ngrd)	
c 
c     m is the suject array index
c
c      sets the outer boundaries of fine grid at position ns
c      using data  from coarse grid at position nb
c
      mb=m+1
      ms=m
c
...
c
      ...
      t2=1.-t1
c
c     write(6,*)'flanks_grds',mb,t1,t2
c
      delx=(grd_xmax(mb)-grd_xmin(mb))/(nx-1.)
      dely=(grd_ymax(mb)-grd_ymin(mb))/(ny-1.)
      delz=(grd_zmax(mb)-grd_zmin(mb))/(nz-1.)
...
c
c    interpolate between main and sub grids
c 
c
c$omp  parallel do 
      do k_n=1,nz_n 
...
c
c       rho_n(i_n,j_n,k_n,m_n)= rho(i,j,k,m)
c
c       write(7,*)i,j,k,m
c          rho_max=amax1(rho_max,rho(i,j,k,m))
c          rho_n_max=amax1(rho_n_max,rho_n(i_n,j_n,k_n,m_n))
          enddo
         enddo
       enddo
c      write(6,*)'refine',rho_max,rho_n_max
c
c      interpolate onto subgrids
c 
c
      do m_n=ngrd_n-1,1,-1
       m=m_n+1
...
c
      ...
      az_n=grd_zmin_n(m_n)+(k_n-1.)*sz
c
c     find position on bigger grid
c
...
c
c     move grid so that it can keep up with an orbiting spacecraft
c
      mx2=mx*2
      my2=my*2
...
c
c       shift array elements
c
c       write(6,*)'shifting xgrd',m_n,n1,n2,isign,n3,n4
c
c$omp  parallel do 
        do k=1,nz_n
        do j=1,ny_n
...
c
c     write(6,*)'y grid shft',m_n,n1,n2,isign,n3,n4
c
c       shift array elements
c
c$omp  parallel do 
        ...
        enddo
c
c       set boundary conditions off bigger grid
c
c
c$omp  parallel do 
        do k_n=1,nz_n 
...
c$omp  parallel do 
        ...
        enddo
c
c       set boundary conditions of main grid
c
c
c$omp  parallel do 
        do k_n=1,nz_n 
...
c       shift array elements
c
c       write(6,*)'shifting ygrd',m_n,n1,n2,isign,n3,n4
c
c$omp  parallel do 
        do k=1,nz_n
        do j=n1,n2,isign
...
c
c    checks for minium rho and negative pressure
c     and resets value if necessary
c
      dimension qrho(nx,ny,nz,ngrd),qpresx(nx,ny,nz,ngrd),
     +          qpresy(nx,ny,nz,ngrd),qpresz(nx,ny,nz,ngrd),
     +          qpresxy(nx,ny,nz,ngrd),qpresxz(nx,ny,nz,ngrd),
...
c
     ...
     +           sin_tilt,cos_tilt 
c
c      determine speeds on the code and changes accordingly
c
      fastest=0.
c
      pxmax=0.
...
c
      ...
      volim=2.*vlim
c
c     vqlim=0.5*sqrt(1.+m)
c     vhlim=0.5*sqrt(1.+m)
c     volim=0.5*sqrt(1.+m)
c
      cqlim=1.0*vqlim
      chlim=1.0*vhlim
      colim=4.0*volim
...
c
c
c       find bulk velocities : species 1
c
c$omp  parallel do
      do k=1,nz
      do j=1,ny
...
c
     ...
     +                +qpresz(i,j,k,m))/arho)
c
c        if (vq.gt.vqlim)then
c         qrho(i,j,k,m)=(rho(ip,j,k)+rho(im,j,k)+rho(i,jp,k)
c    +            +rho(i,jm,k)+rho(i,j,kp)+rho(i,j,km))/6.
c         qpx(i,j,k,m)=qpx(i,j,k,m)*vqlim/vq
c         qpy(i,j,k,m)=qpy(i,j,k,m)*vqlim/vq
c         qpz(i,j,k,m)=qpz(i,j,k,m)*vqlim/vq
c        endif
c
...
c
      ...
      enddo
c
c       find bulk velocities : species 2
c
c$omp  parallel do
      do k=1,nz
      do j=1,ny
...
c
     ...
     +                +hpresz(i,j,k,m))/arho)
c
c        if (vh.gt.vhlim)then
c         hrho(i,j,k,m)=(rho(ip,j,k)+rho(im,j,k)+rho(i,jp,k)
c    +            +rho(i,jm,k)+rho(i,j,kp)+rho(i,j,km))/6.
c         hpx(i,j,k,m)=hpx(i,j,k,m)*vhlim/vh
c         hpy(i,j,k,m)=hpy(i,j,k,m)*vhlim/vh
c         hpz(i,j,k,m)=hpz(i,j,k,m)*vhlim/vh
c        endif
c
...
c
        ...
        enddo
c
c       find bulk velocities : species 3
c
c
c$omp  parallel do
      do k=1,nz
...
c
     ...
     +                +opresz(i,j,k,m))/arho)
c
c        if (vo.gt.volim)then
c         orho(i,j,k,m)=(rho(ip,j,k)+rho(im,j,k)+rho(i,jp,k)
c    +            +rho(i,jm,k)+rho(i,j,kp)+rho(i,j,km))/6.
c         opx(i,j,k,m)=opx(i,j,k,m)*volim/vo
c         opy(i,j,k,m)=opy(i,j,k,m)*volim/vo
c         opz(i,j,k,m)=opz(i,j,k,m)*volim/vo
c        endif
c
...
c
      ...
      enddo
c
c     do electron pressure and alvfen speed
c
...
c       if(alf.gt.alf_lim)then
c         bx(i,j,k,m)=(px(ip,j,k)+px(im,j,k)+px(i,jp,k)
c    +            +px(i,jm,k)+px(i,j,kp)+px(i,j,km))/6.
c         by(i,j,k,m)=(py(ip,j,k)+py(im,j,k)+py(i,jp,k)
c    +            +py(i,jm,k)+py(i,j,kp)+py(i,j,km))/6.
c         bz(i,j,k,m)=(pz(ip,j,k)+pz(im,j,k)+pz(i,jp,k)
c    +            +pz(i,jm,k)+pz(i,j,kp)+pz(i,j,km))/6.
c       endif
c
      enddo
      enddo
