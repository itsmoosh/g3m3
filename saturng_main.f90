...
!     call wtstr(.03,.55,title,1,0,0)
!
!    draw earth and grid references
!
!     reset earth so that it is sitting half way up the axis
!      so that you can see something
!
!     do 190 k=1,mz2
!     do 190 j=1,my
!     do 190 i=1,mx
!      tt(i,j,k+mz2)=tt(i,j,k)
!      tt(i,j,k)=0.
! 190 continue
!
      call gsplci(2)
...
!        ay=xcraft(2,n)/re_equiv
!        az=xcraft(3,n)/re_equiv
!        if( (ax.lt.xmin+abit).or.(ax.gt.xmax-abit).or.
!    +       (ay.lt.ymin+abit).or.(ay.gt.ymax-abit).or.
!    +       (az.lt.zmin+abit).or.(az.gt.zmax-abit) )goto 470
!
!         xp=1.+(ax-xmin)/delx
!         i1=xp+1
!         i2=i1+1
...
!     call wtstr(.3,.98,title,1,0,0)
!     title='y-z plane'
!     call wtstr(.9,.98,title,1,0,0)
!     clev=(fmax-fmin)/(ncon+1.)
!     if(.not.start) call cnrccf(cross,my,my,mz,fmin,
!    +             fmax,clev,0,-1,-1012,2.5,0,1)
!     irecmn=18
!     call conrec(cross,my,my,mz,fmin,
!    +             fmax,clev,0,-1,-1012)
!
!     draw in position of spacecraft
!
...
! 450 continue
!
!      make an equatoiral cut for either imp 8 and geotail
!
! 470 do 490 n=ncraft,2,-1
!        ax=xcraft(1,n)/re_equiv
!        ay=xcraft(2,n)/re_equiv
!        az=xcraft(3,n)/re_equiv
!        if( (ax.lt.xmin+abit).or.(ax.gt.xmax-abit).or.
!    +       (ay.lt.ymin+abit).or.(ay.gt.ymax-abit).or.
!    +       (az.lt.zmin+abit).or.(az.gt.zmax-abit) )goto 500
!
!         xp=1.+(ax-xmin)/delx
!         i1=xp+1
!         i2=i1+1
...
!     clev=(fmax-fmin)/(ncon+1.)
!     if(.not.start) then
!        call cnrccf(along,mx,mx,my,fmin,
!    +             fmax,clev,0,-1,-1012,2.5,0,1)
!     else
!       irecmn=18
!       call conrec(along,mx,mx,my,fmin,
!    +             fmax,clev,0,-1,-1012)
!     endif
!
!     draw in position of spacecraft
...
! 490 continue
!
!      make a 2-d along tail cut for either imp 8 and geotail
!
! 500 do 530 n=ncraft,2,-1
!        ax=xcraft(1,n)/re_equiv
!        ay=xcraft(2,n)/re_equiv
!        az=xcraft(3,n)/re_equiv
!        if( (ax.lt.xmin+abit).or.(ax.gt.xmax-abit).or.
!    +       (ay.lt.ymin+abit).or.(ay.gt.ymax-abit).or.
!    +       (az.lt.zmin+abit).or.(az.gt.zmax-abit) )return
!
!         xp=1.+(ax-xmin)/delx
!         i1=xp+1
!         i2=i1+1
...
!     clev=(fmax-fmin)/(ncon+1.)
!     if(.not.start) then
!        call cnrccf(along,mx,mx,mz,fmin,
!    +             fmax,clev,0,-1,-1012,2.5,0,1)
!     else
!       irecmn=18
!       call conrec(along,mx,mx,mz,fmin,
!    +             fmax,clev,0,-1,-1012)
!     endif
!
!     draw in position of spacecraft
