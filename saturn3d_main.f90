...
!          rad=sqrt(ar**2+az**2)
!          if(
!    +     ( (ar.le.1.5*rearth).and.
!    +       (abs(az).le.0.5*rearth) )
!    +       .or.
!    +      (rad.le.1.25*rearth)
!    +        ) then
...
!      if(divb_hires)then
!        do m=ngrd_n,3,-1
!         write(6,*)'divb_hires',m
!         call divb_correct_n(bx_n,by_n,bz_n,
!    +           bsx_n,bsy_n,bsz_n,btot_n,
!    +           curx_n,cury_n,curz_n,efldx_n,efldy_n,efldz_n,
!    +           7,nx_n*ny_n*nz_n,nx_n,ny_n,nz_n,ngrd_n,m,xspac_n)
!         call divb_correct_n(bx_n,by_n,bz_n,
!    +           bsx_n,bsy_n,bsz_n,btot_n,
!    +           curx_n,cury_n,curz_n,efldx_n,efldy_n,efldz_n,
!    +           7,nx_n*ny_n*nz_n,ngrd_n,nx_n,ny_n,n_nz,m,xspac_n)
!        enddo
!
!        apply bndry conditions
!
!       write(6,*) 'appying boundary conditions'
!        do m=ngrd_n-1,1,-1
!         call flanks_synced(bx_n,nx_n,ny_n,nz_n,ngrd_n,m,
!    +      grd_xmin_n,grd_xmax_n,grd_ymin_n,grd_ymax_n,
!    +      grd_zmin_n,grd_zmax_n)
!         call flanks_synced(by_n,nx_n,ny_n,nz_n,ngrd_n,m,
!    +      grd_xmin_n,grd_xmax_n,grd_ymin_n,grd_ymax_n,
!    +      grd_zmin_n,grd_zmax_n)
!         call flanks_synced(bz_n,nx_n,ny_n,nz_n,ngrd_n,m,
!    +      grd_xmin_n,grd_xmax_n,grd_ymin_n,grd_ymax_n,
!    +      grd_zmin_n,grd_zmax_n)
!        enddo

!       write(6,*) 'appying bndry_corer'
!       do m=1,ngrd_n-1
!         write(6,*)'bndry corer',m
...
!    +       nx_n,ny_n,nz_n,ngrd_n,m,grd_xmin_n,grd_xmax_n,
!    +       grd_ymin_n,grd_ymax_n,grd_zmin_n,grd_zmax_n)
!        enddo
!
!       write(6,*)'applying bndy_grd-core'
!
...
!
!       call mak_dip_all(bx0_n,by0_n,bz0_n,nx_n,ny_n,nz_n,ngrd_n,
!    +               mbndry_n,ijzero_n,numzero_n,mzero_n,rearth,
!    +               grd_xmin_n,grd_xmax_n,grd_ymin_n,grd_ymax_n,
!    +               grd_zmin_n,grd_zmax_n)
!
...
!
!        grd_xmin_n(m_n)=grd_xmin_n(m_n)+dx*lx*2
!        grd_xmax_n(m_n)=grd_xmax_n(m_n)+dx*lx*2
!        grd_ymin_n(m_n)=grd_ymin_n(m_n)+dy*ly*2
!        grd_ymax_n(m_n)=grd_ymax_n(m_n)+dy*ly*2
!
...
!
!     if(m_n.le.2)then
!     write(6,*)'lapidus plot',m_n
!     call visual_hires(wrkqrho_n,
!    +        wrkqpresx_n,wrkqpresy_n,wrkqpresz_n,
!    +        wrkqpx_n,wrkqpy_n,wrkqpz_n,rmassq,
!    +        wrkhrho_n,
!    +        wrkhpresx_n,wrkhpresy_n,wrkhpresz_n,
...
!
!     if(m_n.le.2)then
!     write(6,*)'lapidus before bndry_moon',m_n
!     call visual_hires(wrkqrho_n,
!    +        wrkqpresx_n,wrkqpresy_n,wrkqpresz_n,
!    +        wrkqpx_n,wrkqpy_n,wrkqpz_n,rmassq,
!    +        wrkhrho_n,
...
!     if(m_n.le.2)then
!     write(6,*)'lapidus after set_speed',m_n
!     call visual_hires(wrkqrho_n,
!    +        wrkqpresx_n,wrkqpresy_n,wrkqpresz_n,
!    +        wrkqpx_n,wrkqpy_n,wrkqpz_n,rmassq,
!    +        wrkhrho_n,
...
!     do j=1,ny
!     do i=1,nx
!       efldx(i,j,k)=sqrt(qpresx(i,j,k,m)+epres(i,j,k,m))
!     enddo
!     enddo
!     enddo
!
!      add_dip=.false.
!     call contrace(efldx,bsx,bsy,bsz,nx,ny,nz,ngrd,m,
!    +            xmin,xmax,ymin,ymax,zmin,zmax,1,
!    +            t,'fld pi3',3,11,add_dip,
!    +            radstrt,nphi,pi/3.,theta2,1,
!    +            tx,ty,tz,tg2,work,mx,my,mz,muvwp2,mz)
!
       call clsgks
       end
!
...
!$omp  parallel do
      ...
      do j=1,ny
      do i=1,nx
!      arho=amax1(rho(i,j,k,m),0.005)
!      afld(i,j,k)=gamma1*(erg(i,j,k,m)-0.5*
!    +   (px(i,j,k,m)**2+py(i,j,k,m)**2+pz(i,j,k,m)**2)
!    +                    /arho)
...
!
        ...
        call conhot(efldz,curx,cury,curz,nx,ny,nz,1,1,m, &
             ...
             grd_xmin,grd_xmax,grd_ymin,grd_ymax,grd_zmin,grd_zmax)
!
!       call concraft_fix(efldx,cross,nx,ny,nz,m,
!    +       xcraft,ncraft,re_equiv,ut,'tden yz',start,0.,2.2)
!       call conalong_fix(efldx,along,nx,ny,nz,m,
!    +       xcraft,ncraft,re_equiv,ut,'tden xz',start,0.,2.2)
!       call conplane_fix(efldx,along,nx,ny,nz,m,
!    +       xcraft,ncraft,re_equiv,ut,'tden xy',start,0.,2.2)
!
       enddo
!
       return
