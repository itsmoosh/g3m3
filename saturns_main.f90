...
!        wrkpx(i,j,k,m)=px(i,j,k,m)-fn(i,j,k)+fn(i,j1,k) ! for x-y-z smoothing
      ...
      enddo
!
!         step 3:   diffuse initial in z -dreiction
!
!
!c$omp  parallel do
!     do k=2,nz-1
...
!      px(1,j,nz,m)=wrkpx(1,j,nz,m)
!     enddo
!     enddo
!     calculatethe difference  between x nearest grid points
!
!     do j=1,ny
!     do i=1,nx
!       do k=1,nz-1
!         delta(k)=wrkpx(i,j,k+1,m)-wrkpx(i,j,k,m)   !non-diffuse n+1 solution
!         delta1(k)=px(i,j,k+1,m)-px(i,j,k,m)   !diffused n+1 solution
!        enddo
!        enddo
!
!       delta(nz)=delta(nz-1)
!       delta1(nz)=delta1(nz-1)
!
!       do k=2,nz-1
!        s=sign(1.,delta(k))
!        fn(k)=s*amax1(0.,amin1(s*delta1(k-1),chipx*abs(delta(k)),
!    +                            s*delta1(k+1)))
!       enddo
!
!        fn(1)=fn(2)
...
!
!       do k=2,nz-1
!        px(i,j,k,m)=px(i,j,k,m)-fn(k)+fn(k-1)
!       enddo
!     enddo
!     enddo
!
...
!        k=ijsrf(3,n)
!
!        qrho(i,j,k,m)=parm_srf(m,1,n)
!        hrho(i,j,k,m)=parm_srf(m,2,n)
!        orho(i,j,k,m)=parm_srf(m,3,n)
!        qpres(i,j,k,m)=parm_srf(m,4,n)
!        hpres(i,j,k,m)=parm_srf(m,5,n)*0.9
!        opres(i,j,k,m)=parm_srf(m,6,n)
!        epres(i,j,k,m)=parm_srf(m,7,n)
!
!        qrho(i,j,k,m)=0.5*qrho(i,j,k,m)+0.5*parm_srf(m,1,n)
!        hrho(i,j,k,m)=0.5*hrho(i,j,k,m)+0.5*parm_srf(m,2,n)
!        orho(i,j,k,m)=0.5*orho(i,j,k,m)+0.5*parm_srf(m,3,n)
!
!        qpres(i,j,k,m)=0.5*qpres(i,j,k,m)+0.5*parm_srf(m,4,n)
!        hpres(i,j,k,m)=0.5*hpres(i,j,k,m)+0.5*parm_srf(m,5,n)
!        opres(i,j,k,m)=0.5*opres(i,j,k,m)+0.5*parm_srf(m,6,n)
!        epres(i,j,k,m)=0.5*epres(i,j,k,m)+0.5*parm_srf(m,7,n)
!
!        hpx(i,j,k,m)=0.5*hpx(i,j,k,m)+0.5*rvx*parm_srf(m,2,n)
!        hpy(i,j,k,m)=0.5*hpy(i,j,k,m)+0.5*rvy*parm_srf(m,2,n)
!        hpz(i,j,k,m)=0.5*hpz(i,j,k,m)+0.5*rvz*parm_srf(m,2,n)
!        opx(i,j,k,m)=0.5*opx(i,j,k,m)+0.5*rvx*parm_srf(m,3,n)
!        opy(i,j,k,m)=0.5*opy(i,j,k,m)+0.5*rvy*parm_srf(m,3,n)
!        opz(i,j,k,m)=0.5*opz(i,j,k,m)+0.5*rvz*parm_srf(m,3,n)
!        qpx(i,j,k,m)=0.5*qpx(i,j,k,m)+0.5*rvx*parm_srf(m,1,n)
!        qpy(i,j,k,m)=0.5*qpy(i,j,k,m)+0.5*rvy*parm_srf(m,1,n)
!        qpz(i,j,k,m)=0.5*qpz(i,j,k,m)+0.5*rvz*parm_srf(m,1,n)
!
!
!        opx(i,j,k,m)=orho(i,j,k,m)*rvx
!        opy(i,j,k,m)=orho(i,j,k,m)*rvy
...
!         qpx(i,j,k,m)=qpx(i,j,k,m)*vqlim/vq
!         qpy(i,j,k,m)=qpy(i,j,k,m)*vqlim/vq
!         qpz(i,j,k,m)=qpz(i,j,k,m)*vqlim/vq
!        endif
!
...
!    +            +rho(i,jm,k)+rho(i,j,kp)+rho(i,j,km))/6.
!         hpx(i,j,k,m)=hpx(i,j,k,m)*vhlim/vh
!         hpy(i,j,k,m)=hpy(i,j,k,m)*vhlim/vh
!         hpz(i,j,k,m)=hpz(i,j,k,m)*vhlim/vh
!        endif
!
...
!    +            +rho(i,jm,k)+rho(i,j,kp)+rho(i,j,km))/6.
!         opx(i,j,k,m)=opx(i,j,k,m)*volim/vo
!         opy(i,j,k,m)=opy(i,j,k,m)*volim/vo
!         opz(i,j,k,m)=opz(i,j,k,m)*volim/vo
!        endif
!
