!
!	Calculates the current associated with the perturbed b field
...
subroutine calcur(bx,by,bz,nx,ny,nz,n_grids,box,curx,cury,curz, &
	...
	!	The following boundary conditions are set so there is no forward
	!	communication, i.e. same x required, and symmetry between k=1 &
	!	and k=2.
	...
	curx(nx,ny,nz)=(curx(nx,ny,nz1)+curx(nx,ny1,nz)+curx(nx1,ny,nz))/3.
	...
	cury(nx,ny,nz)=(cury(nx,ny,nz1)+cury(nx,ny1,nz)+cury(nx1,ny,nz))/3.
	...
	curz(nx,ny,nz)=(curz(nx,ny,nz1)+curz(nx,ny1,nz)+curz(nx1,ny,nz))/3.
