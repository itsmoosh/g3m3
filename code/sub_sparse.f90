...
!		ivpt is the position of the bands relative to the diagonal
!
!		n is the order of the original matrix
!		b(n) the real vector for the right hand side
!		x(n) is the solution vector
!
!		rsq is the sum of the squares of the components of the residual
!		vector a.x -b
!
...
recursive subroutine sparse(abd,nband,n,b,x,ipvt,rsq, &
	...
	!	Add the magntiude of the right side and find the residue
	...
	do iter=1,max_iter
		...
		if(rsq.gt.rp)then
			...
			call sparse(abd,nband,n,b,x,ipvt,rsq,g,h,xi,xj,.true.,irst)
		endif
