subroutine set_rho(qrho,qpresx,qpresy,qpresz, &
    ...
    do  k=1,nz
        do  j=1,ny
            do  i=1,nx
                !
                tden=abs(qrho(i,j,k,box))/rmassq+abs(hrho(i,j,k,box))/rmassh &
                +abs(orho(i,j,k,box))/rmasso+1.e-6
                rhdens=abs(hrho(i,j,k,box))/rmassh/tden
                rodens=abs(orho(i,j,k,box))/rmasso/tden
                !
                if ((qrho(i,j,k,box).lt.q_min).or.(qpresx(i,j,k,box).lt.0.).or. &
                    (qpresy(i,j,k,box).lt.0.).or.(qpresz(i,j,k,box).lt.0.))then
                    !
                    qrho(i,j,k,box)=amax1(abs(qrho(i,j,k,box)),1.05*q_min)
                    apres=(abs(qpresx(i,j,k,box))+abs(qpresy(i,j,k,box))+ &
                ...
                !
                if ((hrho(i,j,k,box).lt.h_min).or.(hpresx(i,j,k,box).lt.0.).or. &
                    (hpresy(i,j,k,box).lt.0.).or.(hpresz(i,j,k,box).lt.0.))then
            
                    hrho(i,j,k,box)=amax1(abs(hrho(i,j,k,box)),1.05*h_min)
                    apres=(abs(hpresx(i,j,k,box))+abs(hpresy(i,j,k,box))+ &
                ...
                !
                if ((orho(i,j,k,box).lt.o_min).or.(opresx(i,j,k,box).lt.0.).or. &
                    (opresy(i,j,k,box).lt.0.).or.(opresz(i,j,k,box).lt.0.))then
                
                    orho(i,j,k,box)=amax1(abs(orho(i,j,k,box)),1.05*o_min)
                    apres=(abs(opresx(i,j,k,box))+abs(opresy(i,j,k,box))+ &
