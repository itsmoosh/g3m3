!
!	Reads the name and number of measurements in each spacecraft
!	trajectory file.
!
subroutine names_ntimes(craft_info, scin, ncraft, ndef_craft, &
	...
	!		We use wc -l *.craft so that we get numbers of lines as well
	!		as file names.
	call system ('wc -l '//trim(craft_info)//'*.craft > '// &
	...
	open(scin,file=trim(craft_info)//'crafts.txt', &
		...
		do n=ndef_craft+1,ncraft
			...
			!	Excludes header line in craft file
			ntimes(n,2) = ntimes(n,2) - nheadlines
			!	Step 3: Print the filename before the .craft extension
			!	into craftnames array
			craftnames(n) = junkline( index(junkline,'/')+1 : &
				index(junkline,'.')-1 )
		enddo
	close(scin)
	...
	open(scin+1,file=trim(craft_info)//'defaults.pos', &
		status='unknown',form='formatted')
