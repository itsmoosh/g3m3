!
!	Corrects for possible non-divergence of b;
!	integrates from the wind boundary.
!
subroutine divb_cor(bx,by,bz,px,py,pz,rho,ppres, &
	...
	real bx(nx,ny,nz,n_grids),by(nx,ny,nz,n_grids), &
		bz(nx,ny,nz,n_grids),rho(nx,ny,nz,n_grids), &
		ppres(nx,ny,nz,n_grids), px(nx,ny,nz,n_grids), &
		py(nx,ny,nz,n_grids),pz(nx,ny,nz,n_grids)
	...
	!	Set limit of x integration depending on when you hit
	!	magnetopause
