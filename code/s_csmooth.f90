	...
	do k=2,nz-1
		do j=2,ny-1
			do i=2,nx-1
				...
				fldx(i,j,k)= ( curx(i+1,j,k)+curx(i-1,j,k) + &
					curx(i,j+1,k)+curx(i,j-1,k) &
					+ curx(i,j,k+1)+curx(i,j,k-1) +6.*curx(i,j,k))/12.
				fldy(i,j,k)= ( cury(i+1,j,k)+cury(i-1,j,k) + &
					cury(i,j+1,k)+cury(i,j-1,k) &
					+ cury(i,j,k+1)+cury(i,j,k-1) +6.*cury(i,j,k))/12.
				fldz(i,j,k)= ( curz(i+1,j,k)+curz(i-1,j,k) + &
					curz(i,j+1,k)+curz(i,j-1,k) &
